...
func Start(serverid string, channelid string, contents string, tokens_file string, proxie_file string, threads int, allchannel bool, delay int, allping bool, mentions int) {
...
func sendRequest(url string, contents string, token_file string, proxie_file string) {
	...
	if requests.StatusCode == 200 {
		fmt.Println("[+] Succes to Sent ChannelID:", channel_id, requests.StatusCode, proxy)
	} else {
		fmt.Println("[-] Failed to Sent ChannelID:", channel_id, requests.StatusCode, proxy)
	}
}
